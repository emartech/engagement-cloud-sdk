...
class IosSqDelightEventsDaoTests {
    ...
    private lateinit var db: SapEngagementCloudDB

    @BeforeTest
    fun setup() {
        val driver = NativeSqliteDriver(SapEngagementCloudDB.Schema,
            DB_NAME
        )
    ...
    fun testRemoveEvent_shouldRemoveEvent_ById() = runTest {
        val testEvent = SdkEvent.External.Custom("testId", "test",
            ATTRIBUTES,
            TIMESTAMP
        )
    ...
    fun testRemoveAll_shouldRemoveAllEvents() = runTest {
        val testEvent1 =
            SdkEvent.External.Custom("testId", "test",
                ATTRIBUTES,
                TIMESTAMP
            )
        val testEvent2 =
