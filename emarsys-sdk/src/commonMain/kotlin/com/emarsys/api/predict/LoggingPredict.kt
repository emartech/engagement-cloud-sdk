...
class LoggingPredict(private val logger: Logger) : PredictInstance {
    override suspend fun trackCart(items: List<CartItem>) {
         val entry = LogEntry.createMethodNotAllowed(
            this, this::trackCart.name, mapOf("items" to items)
        )
        logger.debug(entry)
    }
    ...
    override suspend fun trackPurchase(orderId: String, items: List<CartItem>) {
         val entry = LogEntry.createMethodNotAllowed(
            this, this::trackPurchase.name, mapOf("orderId" to orderId, "items" to items)
        )
        logger.debug(entry)
    }
    ...
    override suspend fun trackItemView(itemId: String) {
         val entry = LogEntry.createMethodNotAllowed(
            this, this::trackItemView.name, mapOf("itemId" to itemId)
        )
        logger.debug(entry)
    }
    ...
    override suspend fun trackCategoryView(categoryPath: String) {
         val entry = LogEntry.createMethodNotAllowed(
            this, this::trackCategoryView.name, mapOf("categoryPath" to categoryPath)
        )
        logger.debug(entry)
    }
    ...
    override suspend fun trackSearchTerm(searchTerm: String) {
         val entry = LogEntry.createMethodNotAllowed(
            this, this::trackSearchTerm.name, mapOf("searchTerm" to searchTerm)
        )
        logger.debug(entry)
    }

    override suspend fun trackTag(tag: String, attributes: Map<String, String>?) {
        val params: MutableMap<String, Any> = mutableMapOf("tag" to tag)
        attributes?.let { params.put("attributes", it) }
    ...
    override suspend fun trackRecommendationClick(product: Product) {
        val entry = LogEntry.createMethodNotAllowed(
            this, this::trackRecommendationClick.name, mapOf("product" to product)
        )
        logger.debug(entry)
    }
    ...
    override suspend fun recommendProducts(
        ...
        val params = mapRecommendationParams(logic, filters, limit, availabilityZone)
        
        val entry = LogEntry.createMethodNotAllowed(
            this, this::recommendProducts.name, params
        )
        logger.debug(entry)
        return listOf()
    }
