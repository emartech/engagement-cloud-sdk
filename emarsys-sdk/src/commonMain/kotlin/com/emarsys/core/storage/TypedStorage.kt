...
class TypedStorage(val stringStorage: StringStorageApi, val json: Json, val sdkLogger: Logger): TypedStorageApi {
    override suspend fun <Value>put(key: String, serializer: KSerializer<Value>, value: Value) {
        try {
            ...
            sdkLogger.error("TypedStorage - put", exception)
        }
    }
    ...
    override suspend fun <Value>get(key: String, serializer: KSerializer<Value>): Value? {
        return try {
            ...
            sdkLogger.error("TypedStorage - get", exception)
            null
        }
    }
}
