...
| D-3 | Detekt 2.0's Android type-resolution tasks (`detektMain`/`detektMainAndroid`/`detektHostTestAndroid`/`detektDeviceTestAndroid`) transitively depend on the full Android compilation graph, including `processDebugGoogleServices`, which requires `google-services.json`. Locally unavailable; CI has it via `GOOGLE_SERVICES_JSON_BASE64` secret. **Compounding problem:** detekt 2.0's per-task baseline mechanism writes to `baseline-<task>.xml` next to the configured `baseline` file — they don't merge into our single `baseline.xml`. So even with `google-services.json` in CI, TR findings cannot be absorbed by the single-baseline architecture without a redesigned baseline-merge task. | **v1 deferral.** `detekt-rules-libraries` is set `active: false` in `detekt.yml` (with comments), the rule-set jar stays on the classpath so detekt's class-loader is happy. CI runs only AST-only `./gradlew detekt`. SPEC US-4's marquee rule (`LibraryEntitiesShouldNotBePublic`) is moved to §12 follow-ups. v2 will redesign `detektProjectBaseline` to also invoke per-task TR baseline tasks and merge them into the single file, then re-enable the rule set. |
...
- **Re-enable `detekt-rules-libraries` (TR-only) in v2.** The rule set was deferred from v1 because its findings can't be absorbed by the noJdk single-baseline. v2 work: redesign `detektProjectBaseline` to also invoke per-task TR baseline tasks (`detektBaselineMain`, `detektBaselineMainAndroid`, etc.) and merge their per-task output XMLs into the single `config/detekt/baseline.xml`. CI will then materialize `google-services.json` before the regen step. Once that's in, flip `libraries.active: false` → `true` in `detekt.yml`, regenerate the baseline, and CI gates `LibraryEntitiesShouldNotBePublic` (the SDK API-surface marquee rule) and friends. Track as a separate ticket.
